...
class GCNnet_volleyball(nn.Module):
    ...
    def forward(self, batch_data):
        ...
        images_in_flat = torch.reshape(
            images_in, (B*T, 3, H, W))  # B*T, 3, H, W
        ...
        for features in outputs:
            if features.shape[2:4] != torch.Size([OH, OW]):
                features = F.interpolate(features, size=(
                    OH, OW), mode='bilinear', align_corners=True)
            features_multiscale.append(features)

        features_multiscale = torch.cat(
            features_multiscale, dim=1)  # B*T, D, OH, OW
...
#         boxes_states= torch.cat( [graph_boxes_features,boxes_features],dim=3)  #B, T, N, NFG+NFB
        ...
        if not self.training:
            ...
            actions_scores = torch.mean(actions_scores.reshape(
                B, 3, N, -1), dim=1).reshape(B*N, -1)
            activities_scores = torch.mean(
                activities_scores.reshape(B, 3, -1), dim=1).reshape(B, -1)
...
#         nn.init.zeros_(self.fc_gcn_3.weight)
    ...
    def forward(self, batch_data):
        ...
        boxes_idx = [i * torch.ones(MAX_N, dtype=torch.int)
                     for i in range(B*T)]
        ...
        boxes_features_all = boxes_features_all.reshape(
            B*T, MAX_N, -1)  # B*T,MAX_N, D*K*K
        ...
        for b in range(B):
            ...
            # GSN fusion
            actn_score = torch.mean(actn_score, dim=0).reshape(
                N, -1)  # N, actn_num
            acty_score = torch.mean(acty_score, dim=0).reshape(
                1, -1)  # 1, acty_num
        ...
        if not self.training:
            B = B//3
            actions_scores = torch.mean(
                actions_scores.reshape(-1, 3, actions_scores.shape[1]), dim=1)
            activities_scores = torch.mean(
                activities_scores.reshape(B, 3, -1), dim=1).reshape(B, -1)
